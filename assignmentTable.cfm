...
<cfoutput>
	<div class="form-group">
		<label>Associated Locales:</label>
		<cfif translationManager.hasTranslation(url.contentID,url.siteID)>
			<a class="btn" href="##" onclick="removeTranslationAssignments();return false;">
				Remove Translation Assignment(s)
			</a>
		</cfif>
	</div>
	...
	<table class="table table-striped table-condensed table-bordered mura-table-grid" id="locales">
							...
							<ul class="clearfix navZoom">
								<cfif len(mapping.getRemoteID())>
									...
									and listFindNoCase("Editor,Author",application.permUtility.getnodePerm(mapCrumb))>
										<li class="edit">
											<a title="Edit" target="_top" href="#application.configBean.getContext()#/admin/index.cfm?muraAction=cArch.edit&contenthistid=#mapCrumb[1].contentHistID#&contentid=#mapCrumb[1].contentID#&type=#mapCrumb[1].type#&parentid=#mapCrumb[1].parentID#&topid=#mapCrumb[1].contentID#&siteid=#mapCrumb[1].siteID#&moduleid=00000000000000000000000000000000000&startrow=1" onclick=";">
												<i class="mi-pencil"></i>
											</a>
										</li>
									<cfelse>
		...
		<cfelse>
			<tr>
				<td id="noFilters" colspan="4" class="noResults">
					This site currently has no translation peers.
				</td>
			</tr>
		</cfif>
	</table>
